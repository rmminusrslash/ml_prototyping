...
class SaveBestModelCallback(TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, metrics, **kwargs):
        ...
        if metric_value > self.bestScore:
            eval_logger.info(
                f"** {self.metric_name} score improved from {np.round(self.bestScore, 4)} to {np.round(metric_value, 4)} **"
            )
            self.bestScore = metric_value
            control.should_save = True
        else:
            eval_logger.info(
                f"{self.metric_name} score {np.round(metric_value, 4)} (Prev. Best {np.round(self.bestScore, 4)}) "
            )
